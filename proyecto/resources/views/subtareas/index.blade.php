			...
			<div class="card">
				<div class="card-body">
					<div class="d-flex justify-content-between">
						<h4 class="card-title">Subtareas</h4>
						<div>
...
<script>
    ...
    $(document).ready(function() {
                ...
                table = '<div class="card-body py-2">'+
                    '<ul class="lk-tree-menu">'+
                        '<li>'+
                            ...
                            '<label class="tree_label" for="smCB-1-'+rowData.id+'">'+ (respuesta.nombre != "Sin proyecto raíz" ? respuesta.nombre : "") +'<span class="text-xs text-muted ml-2">'+(respuesta.descripcion == "Proyecto raíz" ? respuesta.descripcion : respuesta.nombre)+'</span></label>'+
                            '<ul>'+
                                '<li>'+
                                    '<input type="checkbox" checked="checked" id="smCB-2-'+rowData.id+'" />'+
                                    '<label class="tree_label" for="smCB-2-'+rowData.id+'">'+respuesta.estado+'<span class="text-xs text-muted ml-2">'+(respuesta.descripcion == "Tarea raíz" ? respuesta.descripcion : "Tarea subraíz")+'</span></label>'+
                                    '<ul>'+
                                        '<li>'+
                                            '<input type="checkbox" checked="checked" id="smCB-3-'+rowData.id+'" />'+
                                            '<label class="tree_label" for="smCB-3-'+rowData.id+'">'+rowData.nombre+'</label>'+
                                            '<ul>'+
                                                '<li>'+
                                                    '<input type="checkbox" checked="checked" id="smCB-4-'+rowData.id+'" />'+
                                                    '<label class="tree_label" for="smCB-4-'+rowData.id+'">Propiedades</label>'+
                                                    '<ul>'+
        ...
        editorTab = $('#dataTab').DataTable({
            ...
            columnDefs: editor_columnsDefs(6),
            columns: editor_columns('id_user'),
            lengthMenu: [
                [10]
            ],
            order: [4, 'asc'],
            ...
            language: lenguaje,
            buttons: editor_buttons(editor,2).concat([{
