            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title">Catálogo de Subdirecciones</h4>
                        <div>
                            <a class="mr-3 pointer d-none d-sm-none d-md-inline-flex"
                                onclick="$('#dataTab').DataTable().ajax.reload();"><i class="fa fa-refresh"></i></a>
                        </div>
                    </div>
                    <p class="mb-4">Presiona <span class="text-success text-bold">nuevo</span> para crear una subdirección. Para
...
<script>
    var editor;
    var editorTab;
    var url = '/SGPI/crud/subdirecciones';
    ...
    $(document).ready(function() {
        ...
        editor = new $.fn.dataTable.Editor({
            ...
            table: '#dataTab',
            idSrc: 'id',
            fields: editor_createSelectForm('Dirección perteneciente','id_dir',array_areas)
        });

        editor.on( 'preSubmit', function ( e, o, action ) {
            if (action !== 'remove' ) {
                var nombre = this.field('nombre');
                var direccion = this.field('id_dir');
                ...
                if (!nombre.isMultiValue()) {
                    ...
                    if (direccion.val() == null) {
                        direccion.error('Debe seleccionar una dirección');
                    }
                    if (estado.val() == 0) {
        ...
        function createChild(row) {
            ...
            for (i = 0; i < array_areas.length; i++) {
                if (rowData.id_dir == array_areas[i].value) {
                    area = array_areas[i].label;
                }
            }
            table = editor_children('Pertenece a la dirección',area,1);
