            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title">Catálogo de Sedes</h4>
                        <div>
                            <a class="mr-3 pointer d-none d-sm-none d-md-inline-flex"
                                onclick="$('#dataTab').DataTable().ajax.reload();"><i class="fa fa-refresh"></i></a>
                        </div>
                    </div>
                    <p class="mb-4">Presiona <span class="text-success text-bold">nuevo</span> para crear una sede. Para
...
<script>
    ...
    $(document).ready(function() {
        ...
        editor = new $.fn.dataTable.Editor({
            ...
            table: '#dataTab',
            idSrc: 'id',
            fields: editor_createDoubleForm('Clave:','ide')
        });

        editor.on( 'preSubmit', function ( e, o, action ) {
            if (action !== 'remove' ) {
                var nombre = this.field( 'nombre' );
                var clave = this.field( 'ide' );
                ...
                if (!nombre.isMultiValue() ) {
                    ...
                    if (!clave.val()) {
                        clave.error('Debe ingresar una clave');
                    }
                    if (clave.val().length >= 10) {
                        clave.error('La longitud debe ser menor a 10 caracteres');
                    }
                    if (estado.val() == 0) {
